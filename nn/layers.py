...
class BiRNN(Layer):
    ...
    def backward(self, out_grad, input):
        ...
        """
        ##############################To Do ##############################
        units = out_grad.shape[-1] // 2
        mask = ~np.any(np.isnan(input), axis=2)

        out_grad_forward = out_grad[:, :, :units]
        in_grad_forward = self.forward_rnn.backward(out_grad_forward, input)
        out_grad_backward = out_grad[:, :, units:]
        in_grad_backward = self.backward_rnn.backward(self._reverse_temporal_data(out_grad_backward, mask),
                                                      self._reverse_temporal_data(input, mask))
        in_grad = np.add(in_grad_forward, self._reverse_temporal_data(in_grad_backward, mask))
        ##################################################################
        return in_grad
